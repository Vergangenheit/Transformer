...
class Encoder(tf.keras.Model):
    ...
    def call(self, pes, sequence, training=True, encoder_mask=None):
        """ Forward pass for the Encoder
                Args:
                    pes: positional encodings
...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, vocab_size, model_size, num_layers, h):
        ...
        self.attention_bot_norm = [tf.keras.layers.BatchNormalization(epsilon=1e-6) for _ in range(num_layers)]
        ...
        self.dense_2 = [tf.keras.layers.Dense(
            model_size) for _ in range(num_layers)]
        self.ffn_dropout = [tf.keras.layers.Dropout(0.1) for _ in range(num_layers)]
        self.ffn_norm = [tf.keras.layers.BatchNormalization(epsilon=1e-6) for _ in range(num_layers)]

        self.dense = tf.keras.layers.Dense(vocab_size)

    def call(self, pes, sequence, encoder_output, training=True, encoder_mask=None):
        ...
        for i in range(self.num_layers):
            ...
            mid_sub_out, mid_alignment = self.attention_mid[i](mid_sub_in, encoder_output, encoder_mask)
