...
def run_test():
    ...
    encoder = model.Encoder(en_vocab_size, config.MODEL_SIZE, config.NUM_LAYERS, config.H)
    sequence_in = tf.constant([[1, 2, 3, 0, 0]])
    encoder_output, _ = encoder(pes, sequence_in)

    # load fr_tokenizer
    with open(config.FR_TOKENIZER, 'rb') as f:
        fr_tokenizer = pickle.load(f)
    fr_vocab_size = len(fr_tokenizer.word_index) + 1
    print(fr_vocab_size)
    decoder = model.Decoder(fr_vocab_size, config.MODEL_SIZE, config.NUM_LAYERS, config.H)
